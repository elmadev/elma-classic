...
void menu_options() {
    ...
    while (true) {
        ...
        nav.add_row(
            ...
            NAV_LAMBDA { State->single = !State->single; });

        if (!State->single) {
            nav.add_row(
                "Flag Tag:", State->flag_tag ? "On" : "Off",
                NAV_LAMBDA { State->flag_tag = !State->flag_tag; });
        }

        nav.add_row("Player A:", State->player1, NAV_LAMBDA { jatekosvalasztas(1, 1); });

        nav.add_row("Player B:", State->player2, NAV_LAMBDA { jatekosvalasztas(0, 1); });

        nav.add_row(
            "Sound:", State->sound_on ? "Enabled" : "Disabled",
            NAV_LAMBDA { State->sound_on = !State->sound_on; });

        nav.add_row(
            "Animated Menus:", State->animated_menus ? "Yes" : "No",
            NAV_LAMBDA { State->animated_menus = !State->animated_menus; });

        nav.add_row(
            "Video Detail:", State->high_quality ? "High" : "Low", NAV_LAMBDA {
        ...
        nav.add_row(
            ...
            NAV_LAMBDA { State->animated_objects = !State->animated_objects; });

        nav.add_row(
            "Swap Bikes:", State->player1_bike1 ? "No" : "Yes",
            NAV_LAMBDA { State->player1_bike1 = !State->player1_bike1; });

        nav.add_row("Customize Controls ...", "", NAV_LAMBDA { menu_customize_controls(); });
        ...
        nav.add_row(
            ...
            NAV_LAMBDA { EolSettings->set_center_camera(!EolSettings->center_camera()); });

        nav.add_row(
            "Centered Minimap:", EolSettings->center_map() ? "Yes" : "No",
            NAV_LAMBDA { EolSettings->set_center_map(!EolSettings->center_map()); });
        ...
        nav.add_row(
            "Zoom:", std::format("{:.2f}", EolSettings->zoom()), NAV_LAMBDA {
        ...
        nav.add_row(
            ...
            NAV_LAMBDA { EolSettings->set_zoom_textures(!EolSettings->zoom_textures()); });
        ...
        nav.add_row(
            ...
            NAV_LAMBDA { EolSettings->set_lctrl_search(!EolSettings->lctrl_search()); });

        nav.add_row("Default LGR:", EolSettings->default_lgr_name(), NAV_LAMBDA { menu_lgr(); });

        nav.add_row(
            "Show Apple Time:", EolSettings->show_last_apple_time() ? "Yes" : "No", NAV_LAMBDA {
