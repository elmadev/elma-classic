...
static int populate_list(const std::string& pattern, int max_filename_length) {
    ListEntries.clear();
    ListEntries.reserve(100);

    finame filename;
    bool done = find_first(pattern.c_str(), filename, max_filename_length);
    int list_length = 0;
    while (!done) {
        ...
        ListEntries.push_back(filename);
...
static void adjust_list_view(int& selected_index, int& view_index, int length,
    ...
    if (wheel != 0 && clicked_box(box_list, Click::None)) {
...
std::string editor_window_open_(bool new_option) {
    ...
    while (true) {
        ...
        if (was_key_just_pressed(DIK_ESCAPE) || clicked_box(box_cancel)) {
            return "";
        } else if (clicked_box(box_new) && new_option) {
            return "";
...
void editor_window_welcome() {
    ...
    std::string filename = editor_window_open_(true);
...
void editor_window_open() {
    ...
    std::string filename = editor_window_open_(false);
...
static void preview_picture(pic8* dest, picture* pict, int x, int y) {
    ...
    width = std::min(width, dest->get_width() - x);
...
bool editor_window_choose_lgr(char* lgrname) {
    ...
    if (list_length < 1) {
        external_error("There is are no LGR files (*.lgr) in the LGR directory!");
    }
...
void editor_window_food_properties(std::string title, object::Property* property, int* animation) {
    invalidateegesz();
    ...
    push();
    render_box(BufferMain, x1, y1, x2, y2, DIALOG_PALETTE_ID, DIALOG_BORDER_PALETTE_ID);
    Pabc2->write_centered(BufferMain, (x1 + x2) / 2, y1 + 20, title.c_str());
    render_box(BufferMain, box_cancel, BUTTON_PALETTE_ID, DIALOG_BORDER_PALETTE_ID);
